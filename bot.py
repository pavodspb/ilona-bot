...
- ОТВЕЧАЙ ТОЛЬКО на основе имеющейся информации выше
- Если 정보 нет в базе — скажи что нужно уточнить у специалиста
- НЕ ВЫДУМЫВАЙ цены, условия, сроки — только то что написано выше
- Если клиент спрашивает про расчёт кухни — предложи /calc
...
async def handle_message(update: Update, context: CallbackContext):
    ...
    calc_keywords = ["рассчитать кухню", "калькулятор кухни", "расчет кухни", "расчёт кухни", "сколько будет стоить моя кухня"]
    if any(kw in lower for kw in calc_keywords):
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("Рассчитать кухню", callback_data="start_calc")]
        ])
        await update.message.reply_text(
            "Хотите рассчитать стоимость вашей кухни?\n\nНажмите кнопку ниже!",
            reply_markup=keyboard
        )
        return
